...
async function renderBoostMatchSelector(boostType) {
  ...
  // 2. Detect if chip already consumed this week (Mon→Mon UTC)
  const nowUtc = new Date();
  const dow = nowUtc.getUTCDay();
  const diffToMon = dow === 0 ? -6 : 1 - dow;
  const weekStart = new Date(Date.UTC(nowUtc.getUTCFullYear(), nowUtc.getUTCMonth(), nowUtc.getUTCDate() + diffToMon));
  const weekEnd = new Date(weekStart);
  weekEnd.setUTCDate(weekEnd.getUTCDate() + 7);

  // Check across ALL predictions (incl. started) to catch chips applied to kicked-off matches
  const usedOnMatch = allMatches.find(p => {
    const md = new Date(p.match.matchDate);
    return md >= weekStart && md < weekEnd &&
      ((boostType === 'JOKER' && p.isDouble) || (boostType === 'SHIELD' && p.isShield));
  });

  // "Resets in Xd HHh" countdown to next Monday
  ...
  content.innerHTML = '<div style="display:flex;flex-direction:column;gap:12px;">' + matches.map(p => {
    ...
    if (isApplied) {
      btnText = 'Applied'; btnBg = '#1DA1F2'; btnColor = '#fff';
      btnCursor = 'pointer'; btnDisabled = ''; btnTitle = 'Remove this chip';
      onclickAttr = `onclick="removeBoostFromMatch('${p.matchId}', '${boostType}', event)"`;
    } else if (isLockedByWeekly) {
      btnText = resetCountdown(); btnBg = 'rgba(255,255,255,0.08)'; btnColor = 'rgba(255,255,255,0.3)';
...
async function selectDemonLeague(leagueId) {
  ...
  try {
    ...
    // Filter out ourselves, filter out users who already have a Demon penalty in this league, and ensure they are targetable (+/- 2 ranks)
    const rivals = data.ranking.filter(r => !r.isMe && !r.hasDemonPenalty && r.isTargetable);
    if (rivals.length === 0) {
