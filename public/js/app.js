...
const TEAM_DISPLAY_MAP = {
  ...
  // MLS
  'Atlanta United':     { name: 'Atlanta United FC', logo: '/images/clubs/atlanta_united.png' },
  'Atlanta United FC':  { logo: '/images/clubs/atlanta_united.png' },
  ...
  try {
    ...
    if (miniListEl) {
      if (!myMiniLeagues.length) {
        ...
        // Fetch each mini league's full detail (which returns prediction-based rankings)
        const miniDetails = await Promise.all(
          myMiniLeagues.map(t =>
            fetch('/api/mini-leagues/' + t.id)
              .then(r => r.ok ? r.json() : null)
              .then(data => ({ tournament: t, data }))
          )
        );

        miniListEl.innerHTML = miniDetails.map(({ tournament: t, data }) => {
          const ranking = data?.ranking || [];
          let me = ranking.find(r => r.isMe);
          if (!me) me = { rank: '-', xp: 0, accuracy: 0 };
          ...
          return '<div style="display:flex;align-items:center;gap:14px;padding:12px;background:rgba(255,255,255,0.03);border-radius:12px;margin-bottom:8px;border:1px solid rgba(255,255,255,0.05);cursor:pointer;transition:background 0.2s" ' +
            ...
            '</div>' +
            '<div style="text-align:right">' +
              '<div style="display:flex;align-items:center;justify-content:flex-end;font-weight:800;font-size:1.1rem;color:' + rankColor + '">#' + me.rank + '<span style="color:rgba(255,255,255,0.2);font-size:0.8rem;margin-left:6px">—</span></div>' +
              '<div style="font-size:0.65rem;color:rgba(255,255,255,0.3)">' + (me.xp || 0).toLocaleString() + ' XP</div>' +
            '</div>' +
            '<div style="color:rgba(255,255,255,0.25);font-size:0.75rem;margin-left:4px">›</div>' +
          '</div>';
        }).join('');
    ...
    if (predsList) {
      if (!preds.length) {
        ...
        predsList.innerHTML = preds.map((pred, idx) => {
          ...
          return `
            <div onclick="openPubProfileBreakdown(${idx})" style="cursor:pointer;background:rgba(255,255,255,0.03);border:1px solid ${correctOutcome ? 'rgba(60,184,46,0.12)' : 'rgba(255,255,255,0.06)'};border-radius:14px;padding:16px 20px;margin-bottom:10px;transition:background 0.15s,border-color 0.15s;" onmouseenter="this.style.background='rgba(255,255,255,0.06)';this.style.borderColor='${correctOutcome ? 'rgba(60,184,46,0.25)' : 'rgba(255,255,255,0.12)'}'" onmouseleave="this.style.background='rgba(255,255,255,0.03)';this.style.borderColor='${correctOutcome ? 'rgba(60,184,46,0.12)' : 'rgba(255,255,255,0.06)'}'">
              ...
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                ...
                <div style="text-align:center;flex-shrink:0;">
                  ...
                  <div style="font-size:0.82rem;font-weight:700;color:${exactScore ? '#FBBF24' : correctOutcome ? '#6FE840' : 'rgba(255,255,255,0.4)'};background:${exactScore ? 'rgba(251,191,36,0.1)' : correctOutcome ? 'rgba(60,184,46,0.1)' : 'rgba(255,255,255,0.04)'};border:1px solid ${exactScore ? 'rgba(251,191,36,0.25)' : correctOutcome ? 'rgba(60,184,46,0.2)' : 'rgba(255,255,255,0.08)'};border-radius:6px;padding:2px 8px;">${pred.homeScore} – ${pred.awayScore}</div>
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;padding-top:10px;border-top:1px solid rgba(255,255,255,0.06);">
                ...
                <div style="font-size:0.85rem;font-weight:800;color:${correctOutcome ? '#6FE840' : (pred.xpEarned || 0) < 0 ? '#F87171' : 'rgba(255,255,255,0.25)'};background:${correctOutcome ? 'rgba(60,184,46,0.1)' : (pred.xpEarned || 0) < 0 ? 'rgba(248,113,113,0.1)' : 'rgba(255,255,255,0.04)'};border:1px solid ${correctOutcome ? 'rgba(60,184,46,0.25)' : (pred.xpEarned || 0) < 0 ? 'rgba(248,113,113,0.25)' : 'rgba(255,255,255,0.08)'};border-radius:100px;padding:4px 12px;font-family:'Russo One',sans-serif;">${correctOutcome ? '+' + pred.xpEarned : (pred.xpEarned || 0) < 0 ? pred.xpEarned : '0'} XP</div>
              </div>
            </div>`;
        }).join('');
      }
...
const STATIC_LOGO_MAP = {
  ...
  'Atlanta United':           '/images/clubs/atlanta_united.png',
  'Atlanta United FC':        '/images/clubs/atlanta_united.png',
  'D.C. United':              '/images/clubs/dc_united.png',
  'Inter Miami':              '/images/clubs/inter_miami.png',
  'Inter Miami CF':           '/images/clubs/inter_miami.png',
  'LA Galaxy':                '/images/clubs/la_galaxy.png',
