...
function LoginFormAr() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res?.error) {
      if (res.error === "EMAIL_NOT_VERIFIED") {
        setError("يرجى التحقق من بريدك الإلكتروني أولاً. تحقق من صندوق الوارد للحصول على رابط التفعيل.");
      } else {
  ...
  return (
    ...
    >
      ...
      {/* ← Home button (RTL: appears on right) */}
      <a
        href="/ar"
        style={{
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "1rem" }}>
            ادخل الملعب وابدأ التنبؤ
          </p>

          {justVerified && (
            <div style={{ background: "rgba(41,191,18,0.12)", border: "1px solid rgba(41,191,18,0.3)", borderRadius: 10, color: "#6FE840", padding: "12px 16px", fontSize: "0.95rem", marginTop: 12, fontWeight: 700 }}>
              ✅ تم التحقق من البريد الإلكتروني! يمكنك الآن تسجيل الدخول.
            </div>
          )}
          {justReset && (
            <div style={{ background: "rgba(41,191,18,0.12)", border: "1px solid rgba(41,191,18,0.3)", borderRadius: 10, color: "#6FE840", padding: "12px 16px", fontSize: "0.95rem", marginTop: 12, fontWeight: 700 }}>
              🔑 تم تحديث كلمة المرور! سجّل الدخول بكلمتك الجديدة.
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {[
              { id: "email",    label: "البريد الإلكتروني", type: "email",    value: email,    onChange: setEmail,    placeholder: "you@example.com", autoComplete: "email" },
              { id: "password", label: "كلمة المرور",        type: "password", value: password, onChange: setPassword, placeholder: "••••••••",        autoComplete: "current-password" },
            ].map((f) => (
              <div key={f.id}>
                <label style={{
                ...
                <input
                  ...
                  style={{
                    ...
                    textAlign: f.type === "email" || f.type === "password" ? "left" : "right",
                  }}
            ...
            <div style={{ textAlign: "left", marginTop: -8, marginBottom: 16 }}>
              <Link href="/forgot-password" style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.88rem", textDecoration: "none", fontWeight: 600 }}>
                نسيت كلمة المرور؟
              </Link>
            </div>
          ...
          <div style={{ marginTop: 20, textAlign: "center", fontSize: "0.95rem", color: "rgba(255,255,255,0.4)" }}>
            ليس لديك حساب؟{" "}
            <Link href="/ar/register" style={{ color: "#6FE840", fontWeight: 700, textDecoration: "none" }}>
              سجّل الآن
            </Link>
          </div>
        </div>
